...
public class Aggregate extends UnaryOperator {
  ...
  protected Schema generateSchema() {
    ...
    for (AggregatorFactory factorie : factories) {
      schema = Schema.merge(schema, factorie.generateSchema(inputSchema));
    }
    return schema;
  }
  ...
  protected void init(final ImmutableMap<String, Object> execEnvVars) throws DbException {
    ...
    for (AggregatorFactory factorie : factories) {
      internalAggs.addAll(factorie.generateInternalAggs(inputSchema, stateSchema.numColumns()));
      emitAggs.addAll(factorie.generateEmitAggs(inputSchema, stateSchema.numColumns()));
      Schema newSchema = factorie.generateStateSchema(inputSchema);
