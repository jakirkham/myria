...
public class WorkerSubQuery extends LocalSubQuery {
          ...
          if (!future.isSuccess()) {
            ...
            if (!(failureReason instanceof QueryKilledException)) {
              ...
              if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                    "got a failed fragment, root op = {} cause {}",
                    drivingFragment.getRootOp().getOpName(),
                    failureReason);
              }
   ...
   */
  public WorkerSubQuery(
      final SubQueryPlan plan, final SubQueryId subQueryId, final Worker ownerWorker) {
  ...
  public final void startExecution() {
    ...
    if (getProfilingMode().contains(ProfilingMode.RESOURCE)) {
      ...
      resourceReportTimer.scheduleAtFixedRate(
          new ResourceUsageReporter(), 0, MyriaConstants.RESOURCE_REPORT_INTERVAL);
    }
    ...
    for (LocalFragment fragment : fragments) {
      if (fragment.getRootOp() instanceof Producer) {
        ...
        for (int i = 0; i < indices.size(); ++i) {
          ...
          newOpId++;
          scan.setOpName(
              "tuplesource for " + fragment.getRootOp().getOpName() + channels[j].getID());
          RecoverProducer rp =
              new RecoverProducer(
                  scan,
                  ExchangePairID.fromExisting(channels[j].getID().getStreamID()),
                  channels[j].getID().getRemoteID(),
                  (Producer) fragment.getRootOp(),
                  j);
          rp.setOpId(newOpId);
          newOpId++;
